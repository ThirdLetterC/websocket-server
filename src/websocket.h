...
typedef struct ws_transport_s {
    ...
    void (*send_raw)(struct ws_transport_s *self, const uint8_t *data, size_t len);
...
typedef struct {
    ...
    void (*on_message)(ws_conn_t *conn, const uint8_t *data, size_t len, ws_opcode_t opcode);
...
/**
 * @brief Feed raw data from the transport (e.g., libuv) into the protocol parser.
 */
